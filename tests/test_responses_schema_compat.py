...
async def test_namespace_tool_choice_uses_flattened_name():
    """A forced namespaced tool choice must name the flattened upstream tool."""
    namespace_tool = {
        "type": "namespace",
        "name": "ns_probe",
        "tools": [{"type": "function", "name": "echo_probe", "description": "echo",
                   "parameters": {"type": "object", "properties": {}}}],
    }
    converter = RequestConverter()
    body, _ = await converter.convert({
        "model": "deepseek-flash",
        "input": "hi",
        "tools": [namespace_tool],
        "tool_choice": {"type": "function", "name": "echo_probe", "namespace": "ns_probe"},
    })

    assert body["tool_choice"] == {
        "type": "function",
        "function": {"name": "ns_probe__echo_probe"},
    }
    assert body["tools"][0]["function"]["name"] == "ns_probe__echo_probe"
